...
<Callout type="info">
The alternative engines listed above (Jinja, Go Templates, Handlebars, Mustache) require custom processor implementation. They are shown here as examples of what you could build, not built-in options.
</Callout>
...
<Callout type="info">
The processor names `custom/jinja`, `custom/go-templates`, and `custom/handlebars` shown in this diagram are illustrative examples. You would need to create custom processors with these names to use alternative templating engines.
</Callout>
...
StartProcessorWithLambda(async (input, fileHelper) => {
  ...
  for (const schema of schemas) {
    ...
    // Modifying 'relative' changes the output file path
...
// Note: resolveAll() copies Copy-type files first, then returns Template-type files
StartProcessorWithLambda(async (input, fileHelper) => {
...
- [Stateless Nature](/docs/developer/processors/explanation/stateless-nature) - Why processors are stateless
- [First Processor](/docs/developer/processors/tutorials/first-processor) - Create your first processor
