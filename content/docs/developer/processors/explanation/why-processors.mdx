...
StartProcessorWithLambda(async (input, fileHelper) => {
  const schemas = fileHelper.read({ // !mark Reads all GraphQL schema files from the 'schemas' directory
    root: 'schemas',
    glob: '**/*.graphql',
    exclude: [],
  ...
  for (const schema of schemas) {
    ...
    schema.relative = schema.relative.replace('.graphql', '.generated.ts'); // !mark Changes output file extension
...
StartProcessorWithLambda(async (input, fileHelper) => {
  ...
  for (const file of files) {
    ...
    const ast = parseTypeScript(file.content); // !mark Parse source to AST
    addDeprecationComments(ast);
    file.content = printTypeScript(ast); // !mark Convert AST back to code
    file.writeFile();
  }
...
StartProcessorWithLambda(async (input, fileHelper) => {
  ...
  const files = fileHelper.resolveAll(); // !mark Copies Copy-type files, then returns Template-type files
...
return {
  processors: [ // !mark Array of processor stages, executed in order
    // Stage 1: Preprocess
    {
