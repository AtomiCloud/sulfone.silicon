...
The `var__name__` syntax is used by the default processor for variable substitution. Variables defined in `config.vars` are accessible using the `var__<variableName>__` pattern. See the [Default Processor](/developer/templates/explanation/default-processor) explanation for more details.
