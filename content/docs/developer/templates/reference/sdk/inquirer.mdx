...
const license = await i.select({
  ...
  default: 'MIT',
  validate: (selected) => {
    if (!selected) return 'Please select a license';
    return null;
  }
});
```
...
const features = await i.checkbox({
  ...
  validate: (selected) => {
    ...
    if (selected.length > 3) return 'Maximum 3 features';
    return null;
  }
});
```
...
All object forms support these properties:
...
| `validate` | `(input) => string \| null` | No | Validation function |
| `options` | `string[]` | For select/checkbox | Available choices |
